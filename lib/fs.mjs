 ...
 * @param {Function} fn
 * @param {String} [opts={}]
 * @param {String} [opts.fs="\t"]
 * @param {String} [opts.rs="\n"]
 * @param {String} [opts.commentChar]
 * @return {void}
 * @uses {@link streamFile}
 */
export async function readTable(path, fn, opts = {}){
	const {fs = "\t", rs = "\n", commentChar} = opts;
	let partial = "";
	await streamFile(path, chunk => {
		...
		for(let line of chunk.split(rs)){
			if(commentChar){
				const comment = line.indexOf(commentChar);
				if(~comment) line = line.slice(0, comment);
			}
			if(line = line.trim())
				fn(...fs ? line.split(fs).map(x => x.trim()) : [line]);
		}
	});

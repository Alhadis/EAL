...
import {decode, encode} from "./shims.mjs";
...
/**
 * Process a file one segment at a time.
 *
 * @param  {String}    path
 * @param  {~callback} [fn]
 * @param  {Object}    [opts]
 * @param  {Number}    [opts.size=1024] - Number of bytes buffered per read operation
 * @param  {Boolean}   [opts.raw=false] - Don't convert processed segments to UTF-8
 * @param  {Number}    [opts.offset=0]  - Offset to commence reading from
 * @param  {FileData}  [opts.rs="\n"]   - Record separator
 * @return {Promise.<?FileData>}
 *    Resolves with any unterminated record data. Unless `raw` was set, data is
 *    converted to a string using UTF-8 encoding (which may not be well-formed).
 */
export async function streamFile(path, fn, opts){
	if("function" !== typeof fn && fn) [opts, fn] = [fn, opts];
	let {rs = [0x0A], size, raw, offset} = opts || {};
	rs     = "string" === typeof rs ? decode(rs) : rs || [];
	offset = Math.max(~~offset, 0) || 0;
	size   = Math.max(~~size, 0)   || 1024;
	...
	const stream  = {
		[Symbol.asyncIterator](){
			...
			return {
				async next(){
					if(eof) return {done: true};
					const bytesRead = await read(fd, chunk, offset);
					offset += bytesRead; eof = bytesRead < size;
					return {done: false, value: chunk.subarray(0, bytesRead)};
				},
			};
		},
	};
	...
	for await(const chunk of stream){
		const {length} = chunk;
		let recordOffset = 0;
		for(let record, eol = 0, i = 0; i < length; ++i){
			if(rs.length && (chunk[i] !== rs[eol] || ++eol < rs.length)) continue;
			if(remainder){ // TODO: Use resizable ArrayBuffers once supported
				record = new Uint8Array(remainder.length + chunk.length);
				record.set(remainder, 0);
				record.set(chunk, remainder.length);
				remainder = null;
			}
			else record = chunk.subarray(recordOffset, i - eol + 1);
			if(!raw) record = encode(record);
			if(fn){
				/**
				 * @callback ~callback
				 * @param {FileData} record - Record data, sans terminator
				 * @param {Number}   index  - Zero-based index of record
				 * @param {Number}   offset - File offset whence the record began
				 */
				record = fn(record, recordIndex, chunkOffset + recordOffset);
				if(undefined !== record) results[recordIndex] = record;
			}
			else results.push(record);
			++recordIndex; recordOffset = i + 1; eol = 0;
		}
		chunkOffset += recordOffset;
		if(recordOffset < length){
			chunkOffset += length - recordOffset;
			remainder = chunk.slice(recordOffset, length);
		}
	}
	await close(fd);
	results.length    = Math.max(results.length, recordIndex);
	results.remainder = !raw && remainder ? encode(remainder) : remainder;
	return results;
}
